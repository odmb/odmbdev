...
// check packet counter for missing packets from DDU
   ...
   // // DEBUG START
   // printk(KERN_INFO "ETH2 Length %d, flag_eevent %d\n",skb->len,flag_eevent_2);
   ...
   if(flag_eevent_2==1){
     ...
     if ( skb->len+SKB_EXTRA-2 == 64 ){
       ...
       end1=*(unsigned short int *)(end_of_packet-DDU_trailer_length+0+20);
       end2=*(unsigned short int *)(end_of_packet-DDU_trailer_length+2+20);
       end3=*(unsigned short int *)(end_of_packet-DDU_trailer_length+4+20);
       end4=*(unsigned short int *)(end_of_packet-DDU_trailer_length+6+20);
       // printk(KERN_INFO " %04d %04x %04x %04x \n",end1&0xffff,end2&0xffff,end3&0xffff,end4&0xffff); 
       //       if((end1==0x8000)&&(end2==0x8000)&&(end3==0xffff)&&(end4==0x8000)){   // look for DCFEB end condition
       if((end1==0xe000)&&(end2==0xe000)&&(end3==0xe000)&&(end4==0xe000)){
	 ...
	 end1=*(unsigned short int *)(end_of_packet-DCC_trailer_length+4);
	 end2=*(unsigned short int *)(end_of_packet-DCC_trailer_length+12);
	 /* for(i=0;i<20;i++){
	    end1=*(unsigned short int *)(end_of_packet-DCC_trailer_length+i);
	    printk(KERN_INFO "%04x",end1&0xffff);
	    } printk(KERN_INFO "\n"); */ 
	 if(((end1&0xff00)==0xef00)&&((end2&0xff00)==0xa000)){
